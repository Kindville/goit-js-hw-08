...
"use strict";var e=a(require("lodash/throttle"));function a(e){return e&&e.__esModule?e:{default:e}}const l="feedback-from-state",t=document.querySelector(".feedback-form"),s=JSON.parse(localStorage.getItem("STORAGE_KEY"));function o(e){e.preventDefault(),""!==t.email.value&&""!==t.message.value?t.email.value&&t.message.value?(console.log({email:`${t.email.value}`,message:` ${t.message.value}`}),localStorage.removeItem("STORAGE_KEY"),e.currentTarget.reset()):console.log("Fill in all the fields"):console.log("There is nothing to submit! Fill up the fields!")}function i(e){localStorage.setItem("STORAGE_KEY",JSON.stringify({email:` ${t.email.value}`,message:` ${t.message.value} `}))}s&&(t.email.value=s.email,t.message.value=s.message),t.addEventListener("submit",o),t.addEventListener("input",(0,e.default)(i,500));
},{"lodash/throttle":"Ywd4"}]},{},["TTyb"], null)
//# sourceMappingURL=/goit-js-hw-08/03-feedback.a163cb47.js.map